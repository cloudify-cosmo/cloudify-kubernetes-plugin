...
class CloudifyKubernetesClient(object):
    ...
    def _prepare_delete_options_resource(self, resource_definition, options):
        if resource_definition.kind != 'ReplicationController':
            if options:
                node_options =\
                    {k: v for k, v in options.items()
                     if hasattr(self.api.V1DeleteOptions, k)}

                return self.api.V1DeleteOptions(**node_options)

            # This is the default should be provided even if the user does not
            # provide the ``propagation_policy`` as part of the option
            return self.api.V1DeleteOptions(**{'propagation_policy': 'Foreground'})
    ...
    def delete_resource(self, mapping, resource_definition,
        ...
        if resource_definition.kind != 'ReplicationController':
            ...
            delete_resource = self._prepare_delete_options_resource(
